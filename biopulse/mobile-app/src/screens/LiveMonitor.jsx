...
import { Activity, Droplets, HeartPulse, Thermometer, Wifi, WifiOff, Zap, Flame } from 'lucide-react';
...
export function LiveMonitor() {
  ...
  const predictedMeta = liveData.mlStressIndex != null
    ? (liveData.mlStressLabel
        ? { ...getStressLabel(liveData.mlStressIndex), label: liveData.mlStressLabel }
        : getStressLabel(liveData.mlStressIndex))
    : null;
  const stressIndex = liveData.mlStressIndex ?? liveData.stressIndex;
  const stressLabel = predictedMeta?.label ?? liveData.label?.label ?? 'WAITING';
  const stressColor = predictedMeta?.color ?? liveData.label?.color ?? '#918fa1';
    ...
    <main style={S.page}>
      ...
      >
        ...
        {liveData.mlConfidence != null && (
          <div style={{ marginTop: 20, padding: '4px 12px', background: 'rgba(108,99,255,0.1)', borderRadius: 12, border: '1px solid rgba(108,99,255,0.3)', display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            <span style={{ fontSize: 13, color: '#e3e0f8', fontWeight: 600 }}>Predicted {stressLabel} ({Math.round((liveData.mlConfidence || 0) * 100)}%)</span>
          </div>
        )}
      </section>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginTop: 16 }}>
        ...
        <VitalsCard
          icon={Zap}
          label="GSR"
          value={liveData.gsr === null ? '--' : Number(liveData.gsr).toFixed(2)}
          unit="%"
          colorHex="#ffb785"
        ...
        <VitalsCard
          icon={Thermometer}
          label="Temp"
          value={liveData.temperature === null ? '--' : Number(liveData.temperature).toFixed(1)}
          unit="C"
          colorHex="#adc6ff"
          bgColorHex="rgba(173,198,255,0.1)"
          status={liveData.temperature === null ? 'Waiting' : 'DHT'}
          sparklineData={historyLine.map((reading) => reading.temperature).filter((value) => value !== null)}
        />
        <VitalsCard
          icon={Droplets}
          label="Humidity"
          value={liveData.humidity === null ? '--' : Number(liveData.humidity).toFixed(1)}
          unit="%"
          colorHex="#adc6ff"
          bgColorHex="rgba(173,198,255,0.1)"
          status={liveData.humidity === null ? 'Waiting' : 'DHT'}
          sparklineData={historyLine.map((reading) => reading.humidity).filter((value) => value !== null)}
        />
        <VitalsCard
          icon={Flame}
          label="Heat Index"
          value={(liveData.temperature !== null && liveData.humidity !== null) 
            ? Number(liveData.temperature + 0.33 * (liveData.humidity / 100 * 6.105 * Math.exp(17.27 * liveData.temperature / (liveData.temperature + 237.3))) - 4.0).toFixed(1) 
            : '--'}
          unit="C"
          colorHex="#ffb785"
          bgColorHex="rgba(255,183,133,0.1)"
          status={(liveData.temperature !== null && liveData.humidity !== null) ? 'Computed' : 'Waiting'}
          sparklineData={historyLine.map((reading) => 
            (reading.temperature !== null && reading.humidity !== null) 
              ? reading.temperature + 0.33 * (reading.humidity / 100 * 6.105 * Math.exp(17.27 * reading.temperature / (reading.temperature + 237.3))) - 4.0 
              : null
